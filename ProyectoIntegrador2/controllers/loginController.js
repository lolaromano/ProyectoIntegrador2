    ...
    login: (req, res) => {
        ...
        users.findOne({
            ...
            .then(user => {
                ...
                } else if (bcrypt.compareSync(req.body.password, user.Password) == false) { //commparo contrasenia que escribo con la qu tengo en la base de datos
                    errors.login = "Contrasenia incorrecta"; //comparamos lo del formulario y user.Password de la base de datos
                    res.locals.error = errors;
                    return res.render('login')
                } else {
